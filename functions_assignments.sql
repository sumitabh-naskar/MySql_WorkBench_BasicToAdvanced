...
SELECT course, name, attendance,
       ROW_NUMBER() OVER (PARTITION BY course ORDER BY attendance DESC) AS course_row_num
FROM Student_Performance;
...
SELECT name, 
       REPLACE(city, 'Mumbai', 'MUM') AS city_abbreviation
FROM Student_Performance;
...
-- ----------ALL THESE FUNCTIONS SEEMS ALIEN TO US, WE ARE SEEING THESE FOR THE FIRST TIME WITH THE HELP OF AI BECAUSE NONE OF THE ABOVE FUNCTIONS USED ARE TAUGHT TO US IN OUR CLASSES -----------------------